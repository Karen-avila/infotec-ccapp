    ...
    mifosX.controllers = _.extend(module, {
        ViewDataTableController: function (scope, routeParams, resourceFactory, location, $uibModal) {
            resourceFactory.DataTablesResource.getTableDetails({ datatablename: routeParams.tableName }, function (data) {
                var temp = [];
                var columnHeaderData = data.columnHeaderData;
                ...
                for (var i = 0; i < columnHeaderData.length; i++) {
                    if (columnHeaderData[i].columnName.indexOf("_cd_") > 0) {
                        temp = columnHeaderData[i].columnName.split("_cd_");
                        columnHeaderData[i].columnName = temp[1];
                        columnHeaderData[i].code = temp[0];
                    }
                }
                scope.datatable = data;
                scope.columnHeaderData = columnHeaderData;
            });
