(function (module) {
  mifosX.controllers = _.extend(module, {
    EditChargeController: function (
      ...
      resourceFactory.chargeResource.getCharge(
        { chargeId: routeParams.id, template: true },
        function (data) {
          scope.template = data;
          ...
          if (data.chargeAppliesTo.value === "Loan") {
            ...
            scope.template.chargeCalculationTypeOptions =
          ...
          scope.chargeCalculationTypeSelected(scope.formData.chargeCalculationType);
      ...
      scope.chargeTimeChange = function (chargeTimeType) {
        if (
          chargeTimeType === 12 &&
          scope.template.chargeAppliesTo.value === "Loan"
        ) {
          scope.showFrequencyOptions = false;
        } else {
          scope.showFrequencyOptions = true;
        ...
        if (scope.formData.chargeAppliesTo === 2) {
          for (var i in scope.template.chargeTimeTypeOptions) {
            if (chargeTimeType === scope.template.chargeTimeTypeOptions[i].id) {
              if (
                scope.template.chargeTimeTypeOptions[i].value == "Annual Fee" ||
      ...
      scope.submit = function () {
        ...
        resourceFactory.chargeResource.update(
          { chargeId: routeParams.id },
          this.formData,
          function (data) {
      ...
      scope.cancel = function () {
        location.path('/viewcharge/' + scope.template.id);
      }
    },
  });
  ...
  mifosX.ng.application
    .controller("EditChargeController", [
      ...
      "$routeParams",
      "dateFilter",
      mifosX.controllers.EditChargeController,
    ])
    .run(function ($log) {
