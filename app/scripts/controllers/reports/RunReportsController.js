(function (module) {
  mifosX.controllers = _.extend(module, {
    RunReportsController: function (
      ...
      resourceFactory.runReportsResource.getReport(
        ...
        function (data) {
          for (var i in data.data) {
            ...
            if (
              temp.displayType == "select" &&
              temp.parentParameterName == null
            ) {
      ...
      function getSuccuessFunction(paramData) {
        var pr =
          routeParams.clientAccountNo == null ||
          routeParams.clientAccountNo == undefined
            ? routeParams.groupAccountNo == null ||
              routeParams.groupAccountNo == undefined
              ? null
              : routeParams.groupAccountNo
            : routeParams.clientAccountNo;
            ...
            for (var j in scope.reportParams) {
              ...
              for (var i in scope.reportParams[j].selectOptions) {
                if (scope.reportParams[j].selectOptions[i].id == pr) {
                  scope.formData[nombReport] =
                    scope.reportParams[j].selectOptions[i].id;
                }
              }
            }
          }
      ...
      function parameterValidationErrors() {
          ...
          switch (paramDetails.displayType) {
            ...
            case "date":
              ...
              if (tmpDate && invalidDate(tmpDate) == true) {
                ...
                errorObj.field = paramDetails.inputName;
                errorObj.code =
                  "error.message.report.invalid.value.for.parameter";
                errorObj.args = { params: [] };
                errorObj.args.params.push({ value: paramDetails.label });
                scope.errorDetails.push(errorObj);
              }
        ...
        if (tmpStartDate > "" && tmpEndDate > "") {
          if (tmpStartDate > tmpEndDate) {
            ...
            errorObj.field = paramDetails.inputName;
            errorObj.code =
              "error.message.report.incorrect.values.for.date.fields";
            errorObj.args = { params: [] };
            errorObj.args.params.push({ value: paramDetails.label });
            scope.errorDetails.push(errorObj);
          }
          ...
          for (var j = 0; j < scope.pentahoReportParameters.length; j++) {
            ...
            if (reqField.name == tempParam.parameterName) {
              ...
              reportParams +=
                ...
                "=" +
                encodeURIComponent(
                  scope.formData[scope.reqFields[i].inputName]
                );
          ...
          switch (scope.reportType) {
            ...
            case "Pentaho":
              ...
              reportURL +=
                ...
                "&locale=" +
                (scope.optlang == undefined || scope.optlang == null
                  ? "es-mx"
                  : scope.optlang.code) +
                "&dateFormat=" +
                (scope.df == undefined || scope.df == null
                  ? "dd MMMM yyyy"
                  : scope.df);
  ...
  mifosX.ng.application
    .controller("RunReportsController", [
      ...
      "$rootScope",
      "$sce",
      mifosX.controllers.RunReportsController,
    ])
    .run(function ($log) {
