(function (module) {
    mifosX.controllers = _.extend(module, {
        NewJLGLoanAccAppController: function (scope, rootScope, routeParams, resourceFactory, location, dateFilter) {
            ...
            scope.loanApplicationCommonData.repaymentsStartingFromDate = new Date();
            scope.loanApplicationCommonData.syncDisbursementWithMeeting = true;
            scope.channelOptions = [];
            scope.paymentFor1000Options = [];
            scope.datatables = [];
            scope.noOfTabs = 1;
            scope.step = '-';
            scope.formData = {};
            ...
            resourceFactory.codeResources.getAllCodes({}, function (data) {
                ...
                scope.paymentFor1000Options = [];
                for (var i in data) {
                    if (data[i].name == "PAYMENT_CHANNEL") {
                        resourceFactory.codeValueResource.getAllCodeValues({codeId: data[i].id}, function (data) {
                            scope.channelOptions = data;
                        });
                    } else if (data[i].name == "PAYMENT_FOR1000") {
                        resourceFactory.codeValueResource.getAllCodeValues({codeId: data[i].id}, function (data) {
                            scope.paymentFor1000Options = data;
                        });
                    }  
                }
            });
                ...
                for (var i in scope.group.clients) {
                    if (scope.group.clients[i].isSelected) {
                        ...
                        if (interestRatePerPeriod > 0) {
                            loanApplication.interestRatePerPeriod = interestRatePerPeriod;
                        } else {
